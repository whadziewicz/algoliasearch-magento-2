...
class ProductHelper extends BaseHelper
{
    ...
    public function getProductCollectionQuery($storeId, $productIds = null, $only_visible = true)
    {
        ...
        if ($only_visible)
            $products = $products->addAttributeToFilter('visibility', array('in' => $this->visibility->getVisibleInSearchIds()));
        ...
        $this->eventManager->dispatch('algolia_rebuild_store_product_index_collection_load_before', array('store' => $storeId, 'collection' => $products));
    ...
    public function setSettings($storeId)
    {
        ...
        if (count($sorting_indices) > 0)
        {
            ...
            $this->algoliaHelper->setSettings($this->getIndexName($storeId), array('slaves' => $slaves));
    ...
    public function getObject(\Magento\Catalog\Model\Product $product)
    {
        ...
        $this->eventManager->dispatch('algolia_subproducts_index', array('custom_data' => $transport, 'sub_products' => $sub_products));
